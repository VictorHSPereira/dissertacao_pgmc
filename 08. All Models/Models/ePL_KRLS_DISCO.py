...
class ePL_KRLS_DISCO:
    ...
    def Initialize_First_Cluster(self, x, y):
        ...
        Theta = Q*y
        self.parameters = pd.DataFrame([[x, x, float(self.hyperparameters['sigma'][0]), np.ones((1,1)), Q, Theta, 0., 1., 0., 1., 1., 1., np.zeros((x.shape[0],1)), 1.]], columns = ['Center', 'Dictionary', 'nu', 'P', 'Q', 'Theta', 'ArousalIndex', 'Utility', 'SumLambda', 'NumObservations', 'TimeCreation', 'CompatibilityMeasure', 'OldCenter', 'tau'])
        Output = y
        self.OutputTrainingPhase = np.append(self.OutputTrainingPhase, Output)
    ...
    def Initialize_Cluster(self, x, y, k, i):
        Q = np.linalg.inv(np.ones((1,1))*(self.hyperparameters.loc[0, 'lambda1'] + (self.Kernel_Gaussiano(x, x))))
        Theta = Q*y
        nu = float(np.linalg.norm(x - self.parameters.loc[i, 'Center'])/math.sqrt(-2 * np.log(max(self.epsilon))))
