...
class ePL_plus:
    def __init__(self, alpha = 0.001, beta = 0.1, lambda1 = 0.35, tau = np.nan, omega = 1000, sigma = 0.25, e_Utility = 0.05, pi = 0.5):
        if tau is np.nan:
            self.hyperparameters = pd.DataFrame({'alpha':[alpha],'beta':[beta], 'lambda1':[lambda1], 'tau':[beta], 'omega':[omega], 'sigma':[sigma], 'e_Utility':[e_Utility], 'pi':[pi]})
        else:
